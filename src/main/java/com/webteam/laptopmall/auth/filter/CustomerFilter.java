package com.webteam.laptopmall.auth.filter;
...
@WebFilter(urlPatterns = {"/api/customer/*"})
public class CustomerFilter implements Filter {
    ...
    private boolean isCustomer(HttpSession session) {
        if (session != null) {
            ...
            if (username != null) {
                ...
                return role.equals(User.ERole.CUSTOMER);
            }
        }
        return false;
    }
    ...
    private void redirectToAccessDeniedPage(HttpServletRequest httpRequest, HttpServletResponse httpResponse)
        ...
        String accessDeniedPage = "/WEB-INF/views/access-denied.jsp";
        httpRequest.getServletContext().getRequestDispatcher(accessDeniedPage).forward(httpRequest, httpResponse);
    }
}
