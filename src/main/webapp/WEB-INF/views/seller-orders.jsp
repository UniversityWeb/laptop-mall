                    ...
                    <div class="product_history-controls row">
                        ...
                        <div class="product_history-controls-right row">
                            <form id="${item.id}" method="post">
                                ...
                                <input type="hidden" name="userId" value="${item.customer.id}">
                                <c:if test="${item.status == 'PENDING'}">
                                    <button class="button-return" name="status" value="PROCESSING" onclick="submitFormWithAction('${item.id}','seller-change-order-status')">Approve</button>
                                    <button class="button-deliver" name="status" value="CANCELLED" onclick="submitFormWithAction('${item.id}','seller-change-order-status')">Cancel</button>
                                </c:if>
                                <c:if test="${item.status == 'PROCESSING'}">
                                    <button class="button-cancel" name="status" value="SHIPPED" onclick="submitFormWithAction('${item.id}','seller-change-order-status')">Ship</button>
                                </c:if>
                                <c:if test="${item.status == 'RETURNED'}">
                                    <button class="button-cancel" name="status" value="REFUNDED" onclick="submitFormWithAction('${item.id}','seller-change-order-status')">Refund</button>
                                </c:if>
                                <button class="button-details" onclick="submitFormWithAction('${item.id}','seller-order-details')">Details</button>
                            </form>
                        </div>
                    </div>
                </div>
